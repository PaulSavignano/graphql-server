...
export const getUserNodeWithFriends = (nodeId) => {
  ...
  const query = tables.users
    ...
    .from(
      tables.users.leftJoin(tables.usersFriends0).on(tables.usersFriends.user_id_a.equals(tables.users.id))
    )
  ...
  return database.getSql(query).then((rows) => {
    ...
    const __friends = rows.map((row) => {
      return {
        user_id_b: row.user_id_b, __tableName: tables.users.getName()
      }
    });
    const source = {
      id: rows[0].id,
...
export const createPost = (body, level, context) => {
  ...
  return database.getSql(query).then(() => {
    return database.getSql({ text: 'SELECT last_insert_rowid() AS if FROM posts'});
  }).then((ids) => {
